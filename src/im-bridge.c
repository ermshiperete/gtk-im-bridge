...
static void
on_ibus_preedit_start(IBusInputContext *ibus_ctx,
                      gpointer user_data)
{
  GtkImBridgeContext *self = GTK_IM_BRIDGE_CONTEXT(user_data);
...
static void
on_ibus_preedit_changed(IBusInputContext *ibus_ctx,
                        gpointer user_data)
{
  GtkImBridgeContext *self = GTK_IM_BRIDGE_CONTEXT(user_data);
...
static void
on_ibus_preedit_end(IBusInputContext *ibus_ctx,
                    gpointer user_data)
{
  GtkImBridgeContext *self = GTK_IM_BRIDGE_CONTEXT(user_data);
                                         ...
                                         int *cursor_pos)
{
  ...
  *cursor_pos = 0;
  ...
  if (self->priv->ibus_context != NULL)
    {
      IBusText *preedit_text = NULL;
      guint cursor_pos_ibus = 0;
      
      g_object_get(self->priv->ibus_context,
                   "preedit-text", &preedit_text,
                   "preedit-cursor-pos", &cursor_pos_ibus,
                   NULL);
      
      if (preedit_text != NULL)
        {
          const char *text = ibus_text_get_text(preedit_text);
          g_free(*str);
          *str = g_strdup(text ? text : "");
          *cursor_pos = cursor_pos_ibus;
